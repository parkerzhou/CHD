$(function() {
	...
	$('#usergrp_dg').datagrid({
		...
		fit : true,
		fitColumns : true,
		url : 'roleAuthController.do?getAllUserGrp',
		method : 'POST',
		columns : [ [ {
			field : 'ug_orderSeq',
			title : '排序',
	...
	$('#role_dg').datagrid({
			...
			fitColumns : true,
			iconCls : 'icon-ok',
			url : 'roleAuthController.do?getRoleInRange',
			toolbar : '#tb',
			method : 'POST',
			columns : [ [
					{
	...
	$('#accept').click(function() {
		var checked_roles = $('#role_dg').datagrid('getChecked');
		var selected_usergrp = $('#usergrp_dg').datagrid('getSelected');
		var rl_id_array = $.map(checked_roles, function(value) {
			return value.rl_id;
		});
		rl_id_array.unshift(0);
		$.post('<%=basePath%>/roleauth_commit.ashx', {
