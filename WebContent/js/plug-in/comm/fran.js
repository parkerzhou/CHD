...
$.extend($.fn.validatebox.defaults.rules,{
	...
	integer:{// 验证输入的大小
		validator:function(value){
			return  /^[0-9]*[1-9][0-9]*$/i.test(value)&& (value <= 32767);
		},
		message : '必须输入正整数且不能大于32767'
	},
...
$(function(){
	...
	simpleFranDataGrid = $('#franDataGrid').datagrid({
		...
		onDblClickRow:function(rowIndex,rowData){
			...
			url = 'franController.do?loadFranById&cmp_seq='+rowData.cmp_seq;
			
			$('#custVisiForm').form('load',url);
...
function delFran(){
	var rows = simpleFranDataGrid.datagrid('getSelections');
	//如果未选中加盟方基本信息，提示用户相关信息
	if(rows.length==0){
		$.messager.alert('提示','请选择要删除的记录！','info');
		return false;
	}
	//如果选中1条或1条以上加盟方基本信息，则提醒用户是否确定删除信息
	if(rows.length>=1){
		//创建并构造加盟方维护信息主键cmp_id数组
		var idArray = [];
		for(var key in rows){
			idArray.push(rows[key].cmp_seq);
		}
		//弹出删除操作选择对话框，如果确定删除加盟方信息，则提交cmp_id数组到后台进行删除操作
...
function openAreaDlg(){
	openCommDialog('区域资料选择','franController.do?getCommPage&page=areaSel',900,480,function(){
		data={};
...
function openCompanySel(){
	openCommDialog('公司资料选择','franController.do?getCommPage&page=cmpSel',900,480,function(){
