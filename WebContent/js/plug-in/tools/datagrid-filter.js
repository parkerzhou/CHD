(function($){
	...
	var oldReloadMethod = $.fn.datagrid.methods.reload;
	$.fn.datagrid.methods.reload = function(jq, data){
		jq.each(function(){
			$.data(this, 'datagrid').filterSource = null;
		});
		return oldReloadMethod.call($.fn.datagrid.methods, jq, data);
	};
	...
	$.extend($.fn.datagrid.defaults, {
		...
		remoteFilter: false,
		filterDefaultText:false,// 默认不显示过滤项
		filterDelay: 400,
		filterRules: [],
	...
	function init(target, filters){
		...
		function createFilter(frozen){
			...
			for(var i=0; i<fields.length; i++){
				...
				if(!fopts && !opts.filterDefaultText){//判断是否默认是否显示过滤项 
					continue;
				}
				...
				if (fopts.op){// 判断是否有定义规则类，未定义的使用默认过滤
