			...
			<div>
				...
				<fieldset>
					...
					<table>
						...
						<tr>
							<td><input name="fran_id_name" id="fran_id_name" style="width:250px"><input id="filterStr" type="hidden" ></td>
...
<script type="text/javascript">
	//全局变量
	var meth ;
	function openFprojFranSelDlg(method,filterStr){
		meth = method;
		if(filterStr!=undefined){
			$("#filterStr").val(filterStr);
		}
			simpleFranDataGrid = $('#fprojFranDataGrid').datagrid({
				queryParams:{fran_id_name:'',filterStr:filterStr},
     ...
     function checkFran(){	
		...
		var fran_id_name=$('#fran_id_name').val();
		    //$("#deptDataGrid").datagrid('reload',{cmp_id_name:cmp_id_name,dept_id_name:dept_id_name});	
			 simpleFranDataGrid = $('#fprojFranDataGrid').datagrid('reload',{fran_id_name:fran_id_name,filterStr:$("#filterStr").val()});	
