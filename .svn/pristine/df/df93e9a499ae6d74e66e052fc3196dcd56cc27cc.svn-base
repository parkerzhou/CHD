...
public class UmmUserInfoServiceImpl implements IUmmUserInfoService{
	...
	//			+ "from sy_UserDet UD,sy_userType UT,sy_User U,sy_userGrp UG,sy_UserAssi UA where U.ut_id=UT.ut_id and UA.ug_id=UG.ug_id and UA.us_id=U.us_id and U.us_id=UD.us_id";
		String sql="select U.us_id,ug_name,us_userId,us_userName,us_nickName,us_email,us_mobileNo,ut_type,st_name,ud_telNo,ud_addr,ud_faxNo "
	...
	public List<User> searchUserInfo(User user){
		String sql="select U.us_id,ug_name,us_userId,us_userName,us_nickName,us_email,us_mobileNo,ut_type,us_state,st_name,ud_telNo,ud_addr,ud_faxNo "
	...
	public boolean addUserInfo(User user,TUserDet userDet,String ug_name,String ut_type){ 
		...
		try{
			jdbcTemplate.update("insert into sy_User(us_id,us_userId,us_userName,us_nickName,us_userPwd,us_email,us_mobileNo,ut_id,us_state,us_dataFrom,us_defLanguage) "
					+ "values(?,?,?,?,?,?,?,?,?,?,?)", new Object[] {us_id,user.getUs_userId(),user.getUs_userName(),user.getUs_nickName(),
							user.getUs_userPwd(),user.getUs_email(),user.getUs_mobileNo(),ut_id,us_state,1,1});
	...
	public boolean alterUserInfo(User user){
		...
		int us_state=jdbcTemplate.queryForInt("select st_id from sy_UserState where st_name='"+user.getUs_state()+"'");
		try{
			jdbcTemplate.update("update sy_User set us_userId=? , us_userName=? , us_nickName=? , us_email=? , us_mobileNo=? , ut_id=? , us_state=?  where us_id=?", 
					new Object[] {user.getUs_userId(),user.getUs_userName(),user.getUs_nickName(),user.getUs_email(),user.getUs_mobileNo(),ut_id,us_state,user.getUs_id()});
