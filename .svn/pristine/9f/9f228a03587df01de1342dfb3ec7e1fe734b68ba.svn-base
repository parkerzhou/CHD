...
import com.glorisun.chd.tag.core.gsui.pojo.Tab;
...
public class TabsTag extends TagSupport {
	...
	public StringBuffer end() {
		...
		if (iframe) {
			...
			sb.append("{");
			sb.append("var s = \'<iframe id=\"\'+id+\'\" scrolling=\"no\" frameborder=\"0\"  src=\"about:chcrm\" width=\""+oConvertUtils.getString(width, "100%")+"\" height=\""+oConvertUtils.getString(heigth, "99.5%")+"\"></iframe>\';");
			sb.append("return s;");
			sb.append("}");
			sb.append("});");
			sb.append("</script>");
		...
		if (tabs) {
			//----------------------------------------------------------------
			//update-end--  Date:20130413 for：自动缩放
			//----------------------------------------------------------------
