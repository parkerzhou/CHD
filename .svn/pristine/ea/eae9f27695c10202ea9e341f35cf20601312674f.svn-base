...
$(function() {
	if (location.href.indexOf("load=detail") != -1) {
		...
		$(":input").attr("style",
				"border:0;border-bottom:1 solid black;background:white;");
	}
});
 ...
 * 报表查询结果显示
 * 
 * @param formObj:需要提交的form对象
 *            url：增加到Tab页面内容的Url
 * @returns
 * 
 */
function repdoSubmit(formObj, url) {
	var qryUrl=url;
	$('#' + formObj).form(
			'submit',
			{
				...
				success : function(data) {
					var d = $.parseJSON(data);
					if (d.success) {
						addOneTab(d.attributes.subtitle, d.attributes.addUrl
								+ '&isIframe&' + d.attributes.fuid + "&"
								+ d.attributes.uId+ "&"+d.attributes.fName, d.attributes.icon);
					} else {
						tip(d.msg);
					}
				}
 ...
 * 
 * @param tit:弹出页面的标题
 *            url：弹出页面的Url objName:要填充的控件(显示名称) objCode:要填充的控件(隐藏Key)
 *            name:列表中checkBox或Radio的名称 fuId:功能ID repId:报表明细选择项ID
 * @returns
 * 
 */
function repPopRadio(tit, url, objName, objCode, name, fuId, repId) {
	$.dialog.setting.zIndex = 2000;
	if (name == "" || url == "") {
		alert("popup参数配置不全");
		return;
	...
	if (typeof (windowapi) == 'undefined') {
		$.dialog({
			content : "url:" + url + "&filterId=" + repId + "&fuid=" + fuId
					+ "&initVal=" + $(objCode).val(),
			lock : true,
			title : tit,
			width : 800,
			...
			ok : function() {
				...
				$("input[name='detItem']:checked", iframe.document).each(
						function() {
				...
				$(objName).val("");
				$(objName).val(names.join(','));

				$(objCode).val("");
				$(objCode).val(codes.join(','));
 ...
 * 
 */
function repPopChkBox(tit, url, objName, objCode, name, fuId, repId) {
	$.dialog.setting.zIndex = 2000;
	if (name == "" || url == "") {
		alert("popup参数配置不全");
		return;
			...
			button : [
					{
						...
						callback : function() {
							...
							$("input[name='detItem']:checked", iframe.document)
									.each(function() {
										var v = $(this).val().split(':::');
										codes.push(v[0]);
										names.push(v[1]);
					...
					},
					{
						...
						callback : function() {
							...
							iframe = this.iframe.contentWindow;
							$("input[name='detItem']", iframe.document).attr(
									"checked", "true");
							return false;
						}
					},
					{
						...
						callback : function() {
							...
							iframe = this.iframe.contentWindow;
							$("input[name='detItem']", iframe.document).each(
									function() {
