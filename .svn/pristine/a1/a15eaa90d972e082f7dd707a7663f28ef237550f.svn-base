	...
	public String delBasic(String cmpqual_seqs) {
		...
		// TODO Auto-generated method stub
			for(String cmpqual_seq : cmpqual_seqs.split(",")){
				...
				String sql = "select count(*) from dbo.jms_performancebond_dtlb where cmpqual_seq = '"+ cmpqual_seq+"'  ";
			
				int count = jdbcTemplate.queryForInt(sql);
				if(count>0){
					return "记录已被引用，不允许删除";
				}
				sql = "select count(*) from jms_qualitymargin_dtlb where cmpqual_seq = '"+cmpqual_seq+"' ";
